...
export class FormularioCarreraComponent implements OnInit {
  ...
  guardarCambios(codigo, nombre, descripcion, codigof){
    ...
    if(this.crear)
      this.data.postCarrera(carrera).subscribe( data => this.router.navigate(['/administracion-carrera']) );
    else
      this.data.putCarrera(carrera).subscribe( data => this.router.navigate(['/administracion-carrera']) );
  }
}
